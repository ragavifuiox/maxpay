...
import 'package:maxpay/view/electricity_bill/cusotmer_electricity.dart';
...
class ConfirmElectricity extends StatefulWidget {
  ...
  const ConfirmElectricity({
    super.key,
    this.productName = 'Electricity Bill',
    this.operatorInitial = 'E',
    this.operatorColor = Colors.orange,
  ...
  @override
  State<ConfirmElectricity> createState() => _ConfirmElectricityState();
}

class _ConfirmElectricityState extends State<ConfirmElectricity> {
  final TextEditingController whatsappController = TextEditingController();
  final TextEditingController amountController = TextEditingController();
  late final ElectricityController controller;
  ...
  void initState() {
    super.initState();
    controller = Get.find<ElectricityController>();
    args = Get.arguments ?? {};

    final String pId = args['product_id'] ?? '';
    final String cId = args['customer_id'] ?? '';

    if (pId.isNotEmpty && cId.isNotEmpty) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        controller.confirmTransaction(productId: pId, consumerNumber: cId);
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final String finalProductName = args['product_name'] ?? widget.productName;
    final rawAmountVal = args['transaction_amount'] ?? 0.0;
    final double rawAmount = rawAmountVal is num
        ? rawAmountVal.toDouble()
        : double.tryParse(rawAmountVal.toString()) ?? 0.0;
    final String logoUrl = args['logo'] ?? '';
    final String customerId = args['customer_id'] ?? 'N/A';
    ...
    return Obx(() {
      ...
      final String transactionAmountStr = rawAmount > 0
          ? rawAmount.toString()
      ...
      final String resolvedProductName =
          confirmData.productName?.isNotEmpty == true
          ? confirmData.productName!
          : finalProductName;
      final String resolvedLogo = confirmData.logo?.isNotEmpty == true
          ? confirmData.logo!
          : logoUrl;
      final String resolvedTxn =
          customerId; // Forced to always use customerId for Transaction No

      final String initial = resolvedProductName.isNotEmpty
          ? resolvedProductName[0]
          : 'E';
      final Color badgeColor = Colors.orange;
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      _buildDetailRow(context, 'Transaction No', resolvedTxn),
                      _buildDetailRow(
                        context,
                        'Available Balance',
                          ...
                          onTap: controller.isPayLoading.value
                              ...
                              : () async {
                                  ...
                                  final result = await controller.payTransaction(
                                    ...
                                    consumerNumber: args['customer_id'] ?? '',
                                    amount: finalAmount.replaceAll('₹', ''),
