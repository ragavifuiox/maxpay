...
//   /// Controls whether the "Proceed" button is shown.
//   /// Becomes true once an amount has been entered (either typed
//   /// manually or filled in by selecting a plan card).
//   bool showNextButton = false;

//   /// Holds the plan/amount the user tapped "Select" on.
//   /// Keys: "amount", "validity", "plan" (SearchDthData)
...
//       Get.find<HomePageController>().walletBalance.value?.data?.balance ?? 0.0;
...
//     productDetailId =
//         selectedPlanData!["plan"].productId.toString();
//   } 
...
//   setState(() {
//     AppLogger.debugPrint("🔍 Operator selected => id: ${value.id}, name: ${value.name}");
//     AppLogger.debugPrint("🔍 Current productId variable BEFORE update => $productId");

//     selectedOperatorObj = value;

//     productId = value.id.toString();   // 👈 THE ACTUAL FIX — was missing
...
//     AppLogger.debugPrint("🔍 productId variable AFTER update => $productId");
//   });
...
//                   // NOTE: Amount field NEVER triggers the search API.
//                   // It only filters the plans already fetched for the
//                   // selected operator (dthcontroller.searchdthList)
//                   // locally, using enteredAmount as the filter key
//                   // (see the "allPlans" building logic in build()).
//                   // onChanged: (value) {
//                   //   dthcontroller.enteredAmount.value = value;

//                   //   // Manual typing invalidates any previously
//                   //   // selected plan card until the user picks again,
//                   //   // and shows the Proceed button once an amount
//                   //   // has been entered.
//                   //   setState(() {
...
//                   //   // 🚫 Do NOT call _triggerSearch() / dthcontroller.searchDth()
//                   //   // here — that would hit the network on every keystroke.
//                   // },
...
//   // 🚫 Do NOT update dthcontroller.enteredAmount here either —
//   // that reactively re-filters the plan list on every keystroke.
// },
//                   onSubmitted: (value) {
//                     // Pressing "Enter/Done" on the keyboard should just
//                     // dismiss the keyboard — it must NOT call the search API.
...
//               await dthcontroller.getCustomerInfo(productId, customerId);
//             },
//           ),
//           SizedBox(width: 10.w),
//           _buildToggleButton(
...
//     final record = (records != null && records.isNotEmpty) ? records.first : null;
...
//                       colors: isDark
//                           ? [AppColors.darkplceholder, AppColors.darkplceholder]
//                           : [const Color(0xFFF2F2F2), const Color(0xFFFFE5EA)],
//                     ),
//                   ),
//                   child: Column(
//                     children: [
//                       _buildInfoRow('Customer Name', record.customername ?? '-', isDark),
//                       _buildInfoRow('Current Balance', record.balance ?? '-', isDark),
//                       _buildInfoRow('Monthly Transaction', record.monthlyrecharge ?? '-', isDark),
//                       _buildInfoRow('Next Transaction', record.nextrechargedate ?? '-', isDark),
//                       _buildInfoRow('Plan Name', record.planname ?? '-', isDark, isMultiLine: true),
//                     ],
//                   ),
//                 ),
//               ),
...
class _DTHRechargePageState extends State<DTHRechargePage>
  ...
  /// 🔹 Scroll + Focus management (keeps focused field visible above keyboard)
