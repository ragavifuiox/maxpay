...
class ProfileController extends GetxController {
  ...
  Future<bool> verifyOtp(String otp) async {
    try {
      ...
      final result = await updateprofileotpusecase(otp, updatedMobile.value);
  ...
  Future<void> deactivateUser() async {
    isLoading.value = true;
    final result = await sl<ActiveUserUsecase>().call("0");
    result.fold(
      (failure) {
        isLoading.value = false;
        CustomToast.error(failure.message);
      ...
      (data) {
        isLoading.value = false;
        CustomToast.success(data.message ?? "User deactivated successfully");
